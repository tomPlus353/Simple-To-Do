    ...
    with ChargingBar(max=len(taskList)) as bar:   #the actual loop where user completes tasks
        while len(taskList) != len(completedTasks):
            ...
            if update.lower() == "stop":
                ...
                completedTasks.append([task for task in taskList if task not in completedTasks][update])
                updateLog(taskList[update])
                #deadline = crossOffDeadline(taskList[update],deadline)
